...
The base case in the function above is 'return if low > high'. When 'low' will exceed 'high', then the function will stop and return without making another recursive call. There will be no infinite recursion. 
...
When we run factorial(10) using this function, the function will step down by 2 each time.
- factorial(10)
- factorial(8)
- factorial(6)
- factorial(4)
- factorial(3)
- factorial(0), etc.
It will continue forever because the base case (n == 1) is never hit. This creates a stack overflow.
...
```
def sum(low, high)
    return low if low == high   # base case
    return high + sum(low, high - 1)
end
```
